...
int main(int argc, char **argv) {
  ...
  int wgrib2(int argc, char **argv) {
    ...
    if (in_file.file_type == NOT_OPEN) {
      ...
      else fatal_error("1 missing input file %s", new_argv[file_arg]);
    }
    ...
    for (;last_message == 0;) {
      ...
      if (mode != 0) fwrite_file(inv_out, 1, strnlen(inv_out,INV_BUFFER), &inv_file);
      ...
      for (j = 0; j < narglist; j++) {
        ...
        if (mode != 0) if (functions[arglist[j].fn].type == inv) fwrite_file(item_deliminator, 1, strlen(item_deliminator), &inv_file);
        if (functions[arglist[j].fn].type != setup) {
          ...
          // if(inv_out[0] != 0)  fprintf(inv_file, "%s", inv_out);
          if (mode != 0) if(inv_out[0] != 0) fwrite_file(inv_out, 1, strnlen(inv_out,INV_BUFFER), &inv_file);
        }
      }
      ...
      if (mode != 0) fwrite_file(end_inv, 1, strlen(end_inv), &inv_file);
